...
class ArionHash:
    ...
    def __init__(self,
        ...
        file = open("G_6.bin", "wb")
        for c_row in self.constants_g:
            for c_value in c_row:
                file.write(c_value.to_bytes("little"))
        file.close()

        file = open("H_6.bin", "wb")
        for c_row in self.constants_h:
            file.write(c_row.to_bytes("little"))
        file.close()

        file = open("Affine_6.bin", "wb")
        for c_row in self.constants_aff:
            for c_value in c_row:
                file.write(c_value.to_bytes("little"))
        file.close()
...
A = ArionHash(branches=8,rounds=4,capacity=4)